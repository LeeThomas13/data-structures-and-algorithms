...
Build a simple express server. Connect a '/hello' route that sends a greeting of your 
 choice. Connect a '/aboutme' route that sends a short bio about you to the front-end. Finally, 
 connect a '/favoritefoods' route that sends an array to the front-end of your favorite foods. All other routes 
 should respond with a status of 404.
------------------------------------------------------------------------------------------------ */
const createServer = () => {
  ...
  app.get("/aboutme", (req, res) => {
    res.send(
      "Hello World, this page is what im all about. My name is name and I love being me. I like my family who I call family. The end."
    );
  });
  let faveFoods = ["pizza", "pizza", "pizza", "pizza", "pizza"];
...
const count = (target, input) => {
  let result = input.reduce((accumOne, value) => {
    let rowCount = value.reduce((accumTwo, value2) => {
      if (value2 === target) {
        return accumTwo + 1;
      }
      return accumTwo;
    }, 0);
    return rowCount + accumOne;
  }, 0);
  return result;
};
/* ------------------------------------------------------------------------------------------------
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = input.map((subInput) => {
    return subInput
      .filter((value) => value % 5 === 0 && typeof value === "number")
      .map((goodValues) => Math.pow(2, goodValues));
  });
  return result;
};
/* ------------------------------------------------------------------------------------------------
